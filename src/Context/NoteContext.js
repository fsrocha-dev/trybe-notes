...
export function NoteProvider({ children }) {
  ...
  const editNote = (id) => {
    const filteredStorageNote = getLocalStorageNotes().find(function (n) { return n.id === id });
    setNote(filteredStorageNote)
  }

  const deleteNote = (id) => {
    const newStorageNotes = getLocalStorageNotes().filter(function (n) { return n.id !== id });
  ...
  const createLocalStorageNote = (newNote) => {
    ...
    if (allNotesStorage === null) {
      ...
      localStorage.setItem("notes", JSON.stringify([...allNotesStorage, newNote]))
    }
  }
  ...
  return (
    <NoteContext.Provider value={{ note, setNote, hasNote, createNote, closeNote, getLocalStorageNotes, editNote, deleteNote }}>
      {children}
